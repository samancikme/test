...
const inputPath = path.join(rootDir, '8soraw.json');
const outputPath = path.join(rootDir, 'src', 'data', 'Level_3_Extra.json');
...
const convertedQuestions = level3Data.map((q, idx) => {
    const qNum = idx + 1;
    const id = `l3e-q${q.id || qNum}`;
    const questionText = q.savol;
    const topic = "Level 3 (Extra)";
